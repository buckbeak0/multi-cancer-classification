...
def predict(image: Image.Image, model_choice: str):
    ...
    if isinstance(preds, dict):
        l1_probs = preds.get('cancer_type', list(preds.values())[0])[0]
        l2_probs = preds.get('subclass', list(preds.values())[1])[0]
    ...
    return {
        ...
        "level1_confidence": level1_conf,
        "level1_probs": l1_all,
        "level2_label": level2_label,
        "level2_confidence": level2_conf,
        "level2_probs": l2_all,
...
async def predict_route(request: Request, file: UploadFile = File(...), model_choice: str = Form("mobilenet")):
    ...
    try:
        ...
        print(f"Level 1 (Cancer Type): {result['level1_label']} [{result['level1_confidence']}%]")
        print(f"Level 2 (Subclass)   : {result['level2_label']} [{result['level2_confidence']}%]")
        print("="*40 + "\n")
